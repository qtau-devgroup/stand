...
private slots:
    void calculate_should_return_the_same_value_as_GetOneFrameResidualSpec()
    {
        ...
        float *fWave = new float[waveLength];
        double *expect = new double[waveLength];
        double *actual = new double[waveLength];
        ...
        for(int i = 0; i < amount; i++)
            calc.calculate(actual, fWave, waveLength / 2, f0, fs, power, waveLength);
...
DECLARE_TEST(PlatinumResidualCalculatorTest)
