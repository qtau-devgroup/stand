...
private slots:
    void StarSpectrumEstimator_estimate_should_return_the_same_value_as_original_star()
    {
        ...
        double **actual = new double *[tLen];
        actual[0] = new double[tLen * fftLength];
        for(int i = 1; i < tLen; i++)
        {
            actual[i] = actual[0] + i * fftLength;
        }
        ...
        for(int i = 0; i < tLen ; i++)
        {
            estimator.estimate(actual[i], fWave, i * msFramePeriod, f0[i], fs, length);
        }
        ...
        for(int i = 0; i < tLen ; i++)
        {
            ...
            {
                double error = fabs(1.0 - (1.0 + actual[i][j]) / (1.0 + expect[i][j]));
                if(error > Error)
                {
                    printf("actual = %e while expected = %e at (i = %d, j = %d)\n", actual[i][j], expect[i][j], i, j);
...
DECLARE_TEST(StarSpectrumEstimatorTest)
